...
def consultar_denominacion(denominacion, clase=""):
    # Segun documentacion oficial: solo necesita <Denominacion>
    # No requiere CUIT ni clave para esta consulta
...
@app.route('/debug/<denominacion>')
def debug(denominacion):
    """Debug: ver que devuelve la API del INPI"""
    try:
        marcas = consultar_denominacion(denominacion.upper(), "")
        return jsonify({
...
def buscar():
    ...
    try:
        ...
        if not denominacion:
            return jsonify({"error": "denominacion requerida", "resultados": [], "total": 0}), 200
        if not INPI_KEY:
            return jsonify({"error": "API INPI no configurada", "resultados": [], "total": 0}), 200

        clases_set = set(int(c) for c in clases)
        resultados = []

        marcas = consultar_denominacion(denominacion, "")
        app.logger.info(f"API devolvio {len(marcas)} marcas para '{denominacion}'")

        for m in marcas:
            if m['clase'] not in clases_set:
                continue
            for c_mia in clases_set:
                rel = rel_clase(c_mia, m['clase'])
                if rel:
        ...
        app.logger.error(f"ERROR en /buscar: {traceback.format_exc()}")
        return jsonify({"error": str(e), "resultados": [], "total": 0, "errores": [str(e)]}), 200
