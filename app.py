...
def buscar():
    ...
    # Buscar sin filtro de clase — la API devuelve todas las clases de una vez
    # Esto es mucho mas rapido que consultar clase por clase
    resultados = []
    errores = []
    clases_set = set(clases)

    try:
        # Una sola consulta sin especificar clase
        marcas = consultar_denominacion(denominacion, "")
        app.logger.info(f"API devolvio {len(marcas)} marcas para '{denominacion}'")
        for m in marcas:
            app.logger.info(f"  Marca: {m['denominacion']} Cl.{m['clase']} Acta:{m['acta']}")
    ...
    return jsonify({
        ...
        "errores": errores[:3] if errores else []
    })
